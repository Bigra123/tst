...
def moch_yayan():
    ...
    try:
        ...
        print '\n %s[%sÃ—%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .ppk/.memek.txt');yayanxd()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%sÃ—%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .ppk/.memek.txt');yayanxd()
...
def publik(kontol):
    ...
    try:
        csy = raw_input('\n %s[%s?%s] id publik1  : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik2 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik3 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik4 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik5 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik6 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik7 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik8 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik9 : '%(N,O,N))
        csy = raw_input('\n %s[%s?%s] id publik10 : '%(N,O,N))
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%sÃ—%s] File [%s%s%s] tidak ada, dump id dulu bro cek menu 1 sampai 4.'%(N,M,N,M,self.apk,N)
